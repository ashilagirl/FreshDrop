...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if(manageTablesBtn) manageTablesBtn.addEventListener('click', manageTables);
  
  // Manage items button
  const manageItemsBtn = document.getElementById('manageItemsBtn');
  if(manageItemsBtn) manageItemsBtn.addEventListener('click', openAddItemModal);
  ...
  if(addTableBtn) addTableBtn.addEventListener('click', addNewTable);
  ...
  if(restockSelectedBtn) {
    restockSelectedBtn.addEventListener('click', () => {
      let name = prompt('Ingredient name:');
  ...
  if(dailyReportBtn) dailyReportBtn.addEventListener('click', dailyReport);
  ...
  const pdfReceiptBtn = document.getElementById('pdfReceiptBtn');
  if(pdfReceiptBtn) pdfReceiptBtn.addEventListener('click', downloadPDF);
  
  const whatsappReceiptBtn = document.getElementById('whatsappReceiptBtn');
  if(whatsappReceiptBtn) whatsappReceiptBtn.addEventListener('click', sendWhatsApp);
  ...
  if(itemForm) itemForm.addEventListener('submit', saveItemForm);
  
  // Add ingredient field button
  const addIngredientFieldBtn = document.getElementById('addIngredientFieldBtn');
  if(addIngredientFieldBtn) {
    addIngredientFieldBtn.addEventListener('click', () => {
      const container = document.getElementById('ingredientsList');
      if(container) {
        const div = document.createElement('div');
        div.className = 'ingredient-row';
        div.innerHTML = `
          ...
          <input type="number" class="ingredient-amount" placeholder="Amount (g/ml)">
          <button type="button" class="remove-ingredient">✖️</button>
        `;
        container.appendChild(div);
  ...
  // Close modals
  const closeButtons = document.querySelectorAll('.close, .close-tables, .close-items, .close-item-form');
  closeButtons.forEach(btn => {
    btn.onclick = () => {
      ...
      if(tablesModal) tablesModal.style.display = 'none';
      if(itemsModal) itemsModal.style.display = 'none';
      if(itemFormModal) itemFormModal.style.display = 'none';
  ...
  if(tablesList && tablesList.parentElement) {
    tablesList.parentElement.appendChild(saveTablesBtn);
  }
});